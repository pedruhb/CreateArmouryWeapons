...
public class SearedPipeAttachmentModel extends BakedModelWrapperWithData {

        private static final ModelProperty<PipeModelData> PIPE_PROPERTY = new ModelProperty<>();

        public SearedPipeAttachmentModel(BakedModel template) {
                super(template);
        }
        ...
        protected ModelData.Builder gatherModelData(ModelData.Builder builder, BlockAndTintGetter world, BlockPos pos, BlockState state, ModelData blockEntityData) {
                ...
                if (transport != null)
                        for (Direction d : Iterate.directions) {
                                ...
                                if (world.getBlockState(pos.relative(d)).getBlock() instanceof FluidPipeBlock) {
                                        if (d.getAxis().isHorizontal()) shouldConnect = world.getBlockState(pos.relative(d)).getValue(PROPERTY_BY_DIRECTION.get(d.getOpposite()));
                                }
                ...
                if (bracket != null) data.putBracket(bracket.getBracket());
        ...
        private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data, PipeModelData pipeData, RenderType renderType) {
                ...
                for (Direction d : Iterate.directions) {
                        ...
                        for (ComponentPartials partial : type.partials) {
                                quads.addAll(AllPartialModels.SEARED_PIPE_ATTACHMENTS.get(partial).get(d).get().getQuads(state, side, rand, data, renderType));
                        }
                }
                
                if (pipeData.isEncased())
                  quads.addAll(AllPartialModels.SEARED_PIPE_CASING.get().getQuads(state, side, rand, data, renderType)); 
