...
public class SearedSpoutRenderer extends SafeBlockEntityRenderer<SearedSpoutBlockEntity> {
	...
	protected void renderSafe(SearedSpoutBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
		...
		for (PartialModel bit : BITS) {
			CachedBufferer.partial(bit, be.getBlockState()).light(light).renderInto(ms, buffer.getBuffer(RenderType.solid()));
